...
public class ContextModels {
	...
	public void modelingTarget() throws Exception {
		...
		Model mdl = procModel(operator.ent("arg/x1", 1.0), operator.ent("arg/x2", 2.0),
				operator.ent("arg/x3", 3.0), operator.ent("arg/x4", 4.0), operator.ent("arg/x5", 5.0));

		add(mdl, operator.ent("invoke", invoker("x1 + x3", operator.ents("x1", "x3"))));
		...
		Double result = (Double) eval(mdl, operator.ent("arg/x1", 2.0), operator.ent("arg/x2", 3.0));
		assertTrue(result.equals(5.0));

		// evaluate the model with new inputs
		add(mdl, operator.ent("invoke", invoker("x6 * x7 + x1", operator.ents("x1", "x6", "x7"))));
		result = (Double) eval(mdl, operator.ent("arg/x6", 6.0), operator.ent("arg/x7", 7.0));
	...
	public void modelDependencies() throws Exception {
		...
		Model mdl1 = procModel(operator.ent("arg/x1", 1.0), operator.ent("arg/x2", 2.0),
				operator.ent("arg/x3", 3.0), operator.ent("arg/x4", 4.0), operator.ent("arg/x5", 5.0));
		add(mdl1, operator.ent("y1", invoker("x1 + x3", operator.ents("x1", "x3"))));
		add(mdl1, operator.ent("y2", invoker("x4 * x5", operator.ents("x1", "x3"))));

		// mdl2 depends on values y1 and y2 calculated in cxt1
		Model mdl2 = procModel(operator.ent("arg/y3", 8.0), operator.ent("arg/y4", 9.0), operator.ent("arg/y5", 10.0));
		add(mdl2, operator.ent("invoke", invoker("y1 + y2 + y4 + y5", operator.ents("y1", "y2", "y4", "y5"))));
...
//		logger.info("result: " + result);
	...
	public void evaluateMultiEntryResponseModel() throws Exception {
		...
		add(mdl, operator.ent("add", invoker("x1 + x3", operator.ents("x1", "x3"))));

		add(mdl, operator.ent("multiply", invoker("x4 * x5", operator.ents("x4", "x5"))));
		...
		logger.info("result: " + result);
		assertTrue(result.equals(context(operator.ent("add", 4.0), operator.ent("multiply", 20.0))));
	}
	...
	public void invokerEntryService() throws Exception {
		...
		Entry ie = operator.ent("multiply", invoker("x1 * x2", operator.ents("x1", "x2")));
