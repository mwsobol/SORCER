...
public class LocalJobExertions implements SorcerConstants {
	...
	public void pathBindingContextScope() throws Exception {
		...
		Task t3 = task("t3", sig("subtract", SubtractorImpl.class),
				cxt("subtract", inVal("arg/z1"), inVal("arg/z2"), outVal("outDispatcher/y")));
		...
		Job job = job(
				...
				pipe(outPoint(t4, "outDispatcher/y"), inPoint(t3, "arg/z1")),
				pipe(outPoint(t5, "outDispatcher/y"), inPoint(t3, "arg/z2")));

		Object result = exec(job);
		logger.info("job outDispatcher: " + result);
	...
	public void executedUpcontextScope() throws Exception {
		...
		Job job = job(
				...
				cxt(inVal("arg/x1", 10.0), inVal("arg/x2", 50.0),
						inVal("arg/y1", 20.0), inVal("arg/y2", 80.0),
						result(outPaths("j1/t3/outDispatcher/y"))),
				job("j2", sig("exert", ServiceJobber.class), t4, t5), t3,
				pipe(outPoint(t4, "outDispatcher/y"), inPoint(t3, "arg/z1")),
				pipe(outPoint(t5, "outDispatcher/y"), inPoint(t3, "arg/z2")));
...
//		logger.info("task context: " + outDispatcher);
		assertTrue(value(result, "outDispatcher/y").equals(100.0));
	...
	public void arithmeticLocalJobExerter() throws Exception {
		...
		Context out  = exert(exerter, context(ent("j1/t3/outDispatcher/y")));
		...
		logger.info("j1/t3/outDispatcher/y: " + out);
		assertEquals(value(out, "j1/t3/outDispatcher/y"), 500.0);
		...
		out = (exert(exerter, invokeContext));
		logger.info("j1/t3/outDispatcher/y: " + out);
		assertEquals(value(out, "j1/t3/outDispatcher/y"), 1210.0);
