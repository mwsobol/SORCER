...
set JINI_BASE=
set JINI_BASE=%LIB_DIR%\river\jsk-platform-%river.version%.jar
set JINI_BASE=%LIB_DIR%\river\jsk-lib-%river.version%.jar
set JINI_BASE=%LIB_DIR%\river\serviceui-%river.version%.jar

set SORCER_COMMON=
set SORCER_COMMON=%LIB_DIR%\common\groovy\groovy-all-%groovy.version%.jar
set SORCER_COMMON=%LIB_DIR%\common\plexus-utils-%plexus.version%.jar
set SORCER_COMMON=%LIB_DIR%\common\jansi-%jansi.version%.jar
set SORCER_COMMON=%LIB_DIR%\common\jline-%$jline.version%.jar

set RIO_CLASSPATH=
set RIO_CLASSPATH=%RIO_HOME%\lib\rio-platform-%rio.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib\rio-lib-%rio.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib\rio-start-%rio.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib-dl\rio-api-%rio.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib-dl\monitor-api-%rio.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib\logging\slf4j-api-%slf4j.version%.jar
set RIO_CLASSPATH=%RIO_HOME%\lib\logging\slf4j-jdk14-%slf4j.version%.jar

set SORCER_PATH=
set SORCER_PATH=%LIB_DIR%\sorcer\lib\sorcer-platform-%sorcer.version%.jar"
set SORCER_PATH=%LIB_DIR%\sorcer\lib\sos-shell-%sorcer.version%.jar"
set SORCER_PATH=%LIB_DIR%\sorcer\lib-ext\webster-%sorcer.version%.jar"
...
rem set JAVA_OPTS=%JAVA_OPTS% -Dorg.rioproject.resolver.jar="%MVN_REPO%"\org\rioproject\resolver\resolver-aether\%v.rio%\resolver-aether-%v.rio%.jar
