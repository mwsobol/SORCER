...
public class OutriggerViewer extends JPanel implements Runnable {
	 ...
	 */
	public OutriggerViewer(Frame f, JavaSpaceAdmin jsa, ServiceBrowserUI sbui) {
		javaSpaceAdmin = jsa;
		frame = f;
		_sbui = sbui;
		initUI();
				...
				Thread t = new Thread(_sbui.wrap(new Runnable() {
					public void run() {
						try {
							...
							JOptionPane.showMessageDialog(panel, ex, "SORCER",
		...
		auto.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				if (_updater != null) {
					...
					auto.setText("    Stop    ");
					// maxEntries=Integer.parseInt(max.getText().trim());
					_updater = new Thread(_sbui.wrap(OutriggerViewer.this));
					_updater.start();
				}
			}
		});
	...
	private void showEntryBrowser(int row) {
		try {
			...
			final AdminIterator iter = javaSpaceAdmin.contents((Entry) tmpl,
					null);
			...
			JOptionPane.showMessageDialog(this, ex, "SORCER",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private JComponent createEntryPanel(final AdminIterator iter,
		...
		del.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				try {
					int ok = JOptionPane.showConfirmDialog(jp,
					...
					JOptionPane.showMessageDialog(jp, ex, "SORCER",
							JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
