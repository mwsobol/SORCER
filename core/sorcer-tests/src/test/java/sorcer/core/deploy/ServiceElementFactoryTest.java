...
public class ServiceElementFactoryTest {
    ...
    public void setDeploymentWebsterOperator() throws IOException, ConfigurationException, URISyntaxException, ResolverException {
        ...
        NetSignature methodEN = new NetSignature("executeFoo",
                                                 ...
                                                 "Yogi");
        methodEN.setDeployment(deploy(configuration(iGridHome + "/bin/sorcer/test/deploy/configs/TestIP.groovy"),
    ...
    public void testConfigurationAsFile() {
        ServiceDeployment deployment = new ServiceDeployment();
        deployment.setConfig(System.getProperty("user.dir")+"/configs/testIP.config");
        Assert.assertNotNull(deployment.getConfig());
        Assert.assertEquals(System.getProperty("user.dir")+"/configs/testIP.config",
    ...
    public void testConfigurationAsHTTP() throws IOException, ConfigurationException {
        ...
        try {
            ...
            deployment.setConfig("http://"+hostName+":"+port+"/sorcer/test/deploy/configs/testIP.config");
            Assert.assertNotNull(deployment.getConfig());
            Assert.assertEquals("http://"+hostName+":"+port+"/sorcer/test/deploy/configs/testIP.config",
                                deployment.getConfig());
        } finally {
            webster.terminate();
        }
