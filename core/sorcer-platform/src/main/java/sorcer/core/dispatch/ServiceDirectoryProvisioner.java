...
public class ServiceDirectoryProvisioner implements Provisioner {
    ...
    public ServiceDirectoryProvisioner() {
        ...
        if (propIdleTime!=null) {
            try {
                ...
                logger.warn("Could not compute property: provisioning.idle.time: " + propIdleTime + " using default idle time of 60 seconds");
            }
        }
    }
    ...
    public <T> T provision(Signature sig) throws ProvisioningException {
        ...
        try {
            ...
            removeFromQueueAndThrowException(provT, "Could not compute operational string", e);
        }
        if (service != null) {
