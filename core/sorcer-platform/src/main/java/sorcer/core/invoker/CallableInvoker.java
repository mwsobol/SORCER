...
public class CallableInvoker<T> extends ServiceInvoker<T> {
	...
	public CallableInvoker(EntModel context) {
		super(context);
	}
	
	public CallableInvoker(EntModel context, Callable callable, Pro... callEntries) {
		super(context);
		this.callable = callable;
		this.args = new ArgSet(callEntries);
	}

	public CallableInvoker(String name, Callable callable, Pro... callEntries) {
		super(name);
		this.callable = callable;
		this.args = new ArgSet(callEntries);
	}
	...
	@Override
	public T compute(Arg... args) throws RemoteException,
			InvocationException {
		return invoke((Context) Arg.selectDomain(args), args);
	}
