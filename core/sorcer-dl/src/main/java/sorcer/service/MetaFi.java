...
public class MetaFi extends Fidelity<Fi> implements Metafidelity, Dependency, Arg, net.jini.core.entry.Entry {
	...
	public MetaFi(String name) {
		this();
		this.fiName = name;
	}

	public MetaFi(String name, String path) {
		this();
		this.fiName = name;
		this.path = path;
	}

    public MetaFi(String name, String path, Fi select) {
        this();
        this.fiName = name;
        this.path = path;
        this.select = select;
    ...
    public MetaFi(String name, List<Fidelity> fis) {
        for (Fidelity fi : fis) {
            selects.add(fi);
        }
        select = fis.get(0);
    ...
    public MetaFi(String name, Fidelity[] fis) {
        for (Fidelity fi : fis) {
            selects.add(fi);
        }
        select = fis[0];
	...
	public MetaFi(String name, String... selects) {
		this.fiName = name;
		fiType = Type.NAME;
		for (String s : selects)
			this.selects.add((Fi) new Tag(s));
